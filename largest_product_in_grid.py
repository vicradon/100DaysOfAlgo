...
def largestProductInGrid(grid):
    ...
    3  [len(grid) - 4, 0], [len(grid) - 3, 1], [len(grid) - 2, 2], [len(grid) - 1, 3]
    ...
    0  [len(grid) - 1, len(grid) - 1]
    1  [len(grid) - 2, len(grid) - 1], [len(grid) - 1,  len(grid) - 2]
    2  [len(grid) - 3, len(grid) - 1], [len(grid) - 2,  len(grid) - 2], [len(grid) - 1,  len(grid) - 3]
    3  [len(grid) - 4, len(grid) - 1], [len(grid) - 3,  len(grid) - 2], [len(grid) - 2,  len(grid) - 3], [len(grid) - 1,  len(grid) - 4]
    ...
    loopLimitArray = [num+1 for num in range(len(grid))]

    leftToRightLinearDiagonals = []
    rightToLeftLinearDiagonals = []
    ...
    for num in loopLimitArray:
        ...
        for index in range(num):
            lTRLinearDiag.append(grid[len(grid) - num + index][index])
            rTLLinearDiag.append(grid[num - index - 1][index])
        leftToRightLinearDiagonals.append(lTRLinearDiag)
        rightToLeftLinearDiagonals.append(rTLLinearDiag)
    ...
    for num in loopLimitArray[::-1][1:]:
        ...
        for index in range(num):
            lTRLinearDiag.append(grid[index][len(grid) - num + index])
            rTLLinearDiag.append(
                grid[len(grid) - index - 1][index + len(grid) - num])
        leftToRightLinearDiagonals.append(lTRLinearDiag)
        rightToLeftLinearDiagonals.append(rTLLinearDiag)

    # diagonal loop
    maxLeftToRightDiag = [0, 0, 0, 0]
    for arr in leftToRightLinearDiagonals:
        for index, nums in enumerate(arr):
            if sum(arr[index:index+4]) > sum(maxLeftToRightDiag) and len(arr) == 4:
                maxLeftToRightDiag = arr[index:index+4]

    maxRightToLeftDiag = [0, 0, 0, 0]
    for arr in rightToLeftLinearDiagonals:
        for index, nums in enumerate(arr):
            if sum(arr[index:index+4]) > sum(maxRightToLeftDiag) and len(arr) == 4:
                maxRightToLeftDiag = arr[index:index+4]

    summaryHashMap = {
        "leftToRight": math.prod(maxRow),
        "topToBottom": math.prod(maxCol),
        "leftToRightdiagonal": math.prod(maxLeftToRightDiag),
        "rightToLeftdiagonal": math.prod(maxRightToLeftDiag),
    }

    return max(summaryHashMap.values())
